...
// Memoized Components
const ShopHeader = memo(({ shop, onBack, router }: any) => (
  <View className="relative h-64">
    ...
    <View className="absolute top-12 right-4 bg-black/50 px-3 py-1 rounded-full flex-row items-center">
      ...
      <Text className="text-primary-light font-semibold ml-1">
...
const ShopInfo = memo(({ shop, distance, onCallShop, onGetDirections }: any) => (
  <MotiView
...
const QueueStatus = memo(({ queueLength, estimatedWaitTime, onViewQueue }: any) => {
  const formatQueueTime = useCallback((minutes: number) => {
...
const ServiceCard = memo(({ service, onBook, index }: any) => (
  <MotiView
    key={service.id}
...
const NoShopView = ({ onBack }: { onBack: () => void }) => (
  <View className="flex-1 bg-dark-background justify-center items-center">
...
const ShopDetailScreen = () => {
  ...
  const loadShopDetails = async () => {
    try {
      ...
      const [shopResult, servicesResult, queueResult] = await Promise.all([
        supabase.from('shops').select('*').eq('id', id).single(),
        supabase.from('services').select('*').eq('shop_id', id).eq('is_active', true).order('price'),
        supabase.from('queue').select('*').eq('shop_id', id).eq('status', 'waiting').order('position')
      ]);
  ...
  if (!shop) {
    return <NoShopView onBack={() => router.back()} />;
  }
  ...
  return (
    ...
    >
      <ShopHeader shop={shop} router={router} />
