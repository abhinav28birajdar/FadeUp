import React, { useState, useEffect } from 'react';
import { View, StyleSheet, FlatList, ActivityIndicator } from 'react-native';
import { useRouter } from 'expo-router';
import { Colors } from '../../constants/colors';
import { Spacing } from '../../constants/spacing';
import { useBookingStore } from '../../store/booking.store';
import { shopService } from '../../services/shop.service';
import { Service } from '../../types/firestore.types';
import { ScreenHeader } from '../../components/ui/ScreenHeader';
import { ServiceCard } from '../../components/ui/ServiceCard';
import { Button } from '../../components/ui/Button';

export default function SelectServiceScreen() {
    const router = useRouter();
    const { selectedShop: shop, selectedService: service, setSelectedService: setService } = useBookingStore();
    const [services, setServices] = useState<Service[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    useEffect(() => {
        if (shop) {
            shopService.getShopServices(shop.id)
                .then(data => {
                    setServices(data);
                    setIsLoading(false);
                })
                .catch(() => setIsLoading(false));
        }
    }, [shop]);

    if (isLoading) {
        return <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}><ActivityIndicator color={Colors.primary} /></View>;
    }

    return (
        <View style={styles.container}>
            <ScreenHeader title="Select Service" />

            <FlatList
                data={services}
                keyExtractor={item => item.id}
                contentContainerStyle={styles.list}
                renderItem={({ item }) => (
                    <ServiceCard
                        service={item}
                        isSelected={service?.id === item.id}
                        onSelect={() => setService(item)}
                    />
                )}
            />

            <View style={styles.footer}>
                <Button
                    label="Next step"
                    onPress={() => router.push('/booking/select-barber')}
                    disabled={!service}
                    fullWidth
                />
            </View>
        </View>
    );
}

const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: Colors.background },
    list: { padding: Spacing.xl },
    footer: { padding: Spacing.xl, backgroundColor: Colors.surface, borderTopWidth: 1, borderTopColor: Colors.border },
});
